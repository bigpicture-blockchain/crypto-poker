...
export class ApiService {
  ...
  async getWsUri(): Promise<string> {
    ...
    let signature = new URL(window.location.href).searchParams.get("signature")

    if (guid && message && signature) {
      const signerAddr = await ethers.utils.verifyMessage(message, signature);
      if (signerAddr !== guid) {
        window.location.replace("https://dev.badbeat.com/");
        return
      }
    }
    else{
